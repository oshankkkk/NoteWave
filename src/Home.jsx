...
function Home() {
    ...
    chunks.forEach(chunk => {
      ...
      const unsubscribeGroup = onSnapshot(q, (querySnap) => {
        querySnap.forEach(doc => {
          ...
          allGroupsMap[doc.id] = { id: doc.id, ...data, unreadCount };
        });
  ...
  const resetUnreadCount = async (groupId) => {
    if (!user) return;
        ...
        <ul id="grp-container">
          ...
          {groups.map(group => (
            ...
            <div key={group.id} className="img-msg" onClick={() => resetUnreadCount(group.id)}>
              ...
              {group.Admin==user.uid &&( <button id="admin-edit"><i class="fa-solid fa-pen"></i></button>)}
